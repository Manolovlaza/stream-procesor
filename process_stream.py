...
if not api_url or not api_key:
    print("❌ ERROR: API_URL and API_KEY secrets are missing from environment.")
    exit(1)

print(f"🚀 Iniciando procesamiento para: {stream_url}")

# 1. Descarga ultra-rapida de audio nativo con yt-dlp
...
# 2. Troceado instantaneo en bloques de 45 minutos manteniendo la extension nativa
...
print(f"📦 Creados {len(chunks)} bloques de audio ({ext}) para transcripcion paralela.")

# 3. Transcripcion en paralelo usando el pool de Cloudflare
...
def transcribe_chunk(chunk_file, idx):
    ...
    if resp.status_code == 200:
        ...
        print(f"  * Bloque {idx:02d} ({chunk_file}): HTTP 200 OK en {elapsed:.2f}s ({len(segments)} subtitulos)")
        return idx, segments
    else:
...
# 4. Generacion del archivo Master SRT final
...
print(f"📄 Subtitulos Maestro generados con {total_b} bloques de tiempo sincronizados.")
